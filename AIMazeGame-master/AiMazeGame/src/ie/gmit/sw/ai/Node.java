...
public class Node {
	...
	public boolean hasDirection(Direction direction) throws NullPointerException{	
		try {
	...
	public Node[] adjacentNodes(Node[][] maze){
		...
		if (row > 0) adjacents.add(maze[row - 1][col]); 
		if (row < maze.length - 1) adjacents.add(maze[row + 1][col]); 
		if (col > 0) adjacents.add(maze[row][col - 1]);
		if (col < maze[row].length - 1) adjacents.add(maze[row][col + 1]); 
