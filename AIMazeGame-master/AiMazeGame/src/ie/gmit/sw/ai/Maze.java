...
public class Maze {
	...
	// generation algorithims used in lap
	private Node[][] maze;
	private Node goal;
	private int goalPos;
	...
	// pads the edge of the maze with walls
	private void buildMaze() {
		for (int row = 0; row < maze.length; row++) {
			for (int col = 0; col < maze[row].length - 1; col++) {
				int num = (int) (Math.random() * 10);
				if(row == 0 || row == (maze.length -1) || col == 0 || col == maze.length) {
					maze[row][col].setNodeType('X');
					maze[row][col].setWalkable(false);
				}else if (num >= 5 && col + 1 < maze[row].length - 1) {
					// free path avalible for player to walk on 
	...
	public void setGoalNode() { // sets the exit randomly in the map 
